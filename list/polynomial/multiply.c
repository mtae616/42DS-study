...
LinkedList	*multiply(LinkedList *aList, LinkedList *bList) // 다항식의 곱셈
{
	...
	while(alen--) {
		...
		while(blen--) {
			newNode->coef = aHeadNode->coef * bHeadNode->coef; // 각 계수는 곱해주고
			newNode->degree = aHeadNode->degree + bHeadNode->degree; // 각 차수는 더해준다.
